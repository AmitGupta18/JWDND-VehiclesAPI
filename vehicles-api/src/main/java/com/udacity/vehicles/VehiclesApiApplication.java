...
public class VehiclesApiApplication {
	...
	public WebClient webClientPricing(@Value("${pricing.endpoint}") String endpoint,
			@Value("${pricing.service.name}") String pricingService) {
		List<InstanceInfo> instances = eurekaClient.getApplication(pricingService).getInstances();
		if (instances == null || instances.size() <= 0) {
			return WebClient.create(endpoint);
		}

		return WebClient.create("http://" + instances.get(0).getHostName() + ":" + instances.get(0).getPort());
	}
